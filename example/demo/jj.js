...
module.exports.prom = ff.lazy.printSelect.do(async (res) => {
    ...
    const menu = {
        a: ['a menu', ff.useIn.resetMenu.stay.do(async () => {
            const rl = await jj.rl('Type name: ', { password: true });
...
module.exports.sub_menu_test = [`sub menu test description`, {
    run: ['run comment', ss('bash -c'), {
        a: ss(['echo if only one string param exist that will be the command']),
        b: ss(['echo if string is specified the program join them and run']),
    ...
    find: [`find files desc`, ss(`find . -type f -exec echo file from jj.js: {} ;`)],
    roo: ss("echo simple run an echo"),
}]
    ...
    setTimeout(async () => {
        res({
            all_split: ff.stay.do(async () => {
                const a = await jj.cli.splitAll.do('sh -c', [`echo ${false} hi && echo other done other`]);
                console.log(a.o[0][0]);
            }),
